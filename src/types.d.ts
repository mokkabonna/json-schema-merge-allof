...
declare namespace merger {
  interface Options<Schema extends JSONSchema = JSONSchema> {
     ...
     * schema spec. Be aware though that the schema produced will allow more
     * than the original schema. But this is useful if just want to combine
     * schemas using allOf as if additionalProperties wasn't false during the
     * merge process. The resulting schema will still get additionalProperties
     * set to false.
     */
     ...
     * the schema that caused the resolver to be called (useful if you use the
     * same resolver for multiple keywords, or want to implement specific logic
     * for custom paths) - **mergeSchemas** a function you can call that merges
     * an array of schemas - **options** the options mergeAllOf was called with
     */
    resolvers?: Partial<Resolvers<Schema>> & {
      /**
       * ### Default resolver You can set a default resolver that catches any
       * unknown keyword. Let's say you want to use the same strategy as the
       * ones for the meta keywords, to use the first value found. You can
  ...
  interface Resolvers<Schema extends JSONSchema = JSONSchema> {
    ...
    /**
     * ### Combined resolvers No separate resolver is called for
     * patternProperties and additionalProperties, only the properties resolver
     * is called. Same for additionalItems, only items resolver is called. This
     * is because those keywords need to be resolved together as they affect
     * each other.
     *
     ...
     *     additionalProperties: ..., } ```
     *
     * Also the resolve function is not passed **mergeSchemas**, but an object
     * **mergers** that contains mergers for each of the related keywords. So
     * properties get passed an object like this:
     *
     * ```js var mergers = {
     ...
     *
     * Some of the mergers requires you to supply a string of the name or index
     * of the subschema you are currently merging. This is to make sure the
     * path passed to child resolvers are correct.
     */
    properties(
      values: Schema[],
      path: string[],
