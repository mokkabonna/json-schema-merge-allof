...
describe('module', function() {
  it('does not mutate original schema', function() {
    var schema = {
      allOf: [
        {
          type: 'object',
    ...
    expect(origSchema).to.eql(schema)
    expect(result === schema).to.equal(false)
    expect(result === schema.allOf[0]).to.equal(false)
    expect(result.properties === schema.allOf[0].properties).to.equal(false)
    expect(result.properties.name === schema.allOf[0].properties.name).to.equal(false)
    expect(result === schema.allOf[1]).to.equal(false)
